 ...
 */ export function resolve(...pathSegments) {
  ...
  for (let i = pathSegments.length - 1; i >= -1; i--) {
    ...
    if (i >= 0) {
      ...
      if (
        path === undefined || path.slice(0, 3).toLowerCase() !== `${resolvedDevice.toLowerCase()}\\`
      ) {
        path = `${resolvedDevice}\\`;
      }
    }
