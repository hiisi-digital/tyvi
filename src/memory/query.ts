...
import type { Memory, MemoryFilters, MemoryQuery, MemorySummary } from "../types/mod.ts";
...
function matchesQuery(memory: Memory, query: MemoryQuery, now: Date): boolean {
  ...
  if (query.topic) {
    ...
    const hasMatchingTopic = topics.some((topic) => memory.tags.topics.includes(topic));
    if (!hasMatchingTopic) {
      return false;
    }
  }
  ...
  if (query.people) {
    ...
    const hasMatchingPerson = people.some((person) => memory.tags.people.includes(person));
...
export function queryMemories(
  ...
  // Filter memories
  const results = allMemories.filter((memory) => matchesQuery(memory, query, now));
