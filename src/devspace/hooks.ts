...
export function generateHook(hookType: "pre-commit"): string {
  switch (hookType) {
    case "pre-commit":
      return `#!/bin/sh
# tyvi git guard — pre-commit hook
# Blocks direct commits to the devspace project unless explicitly allowed.
# Auto-generated by tyvi. Do not edit manually.

# Allow if TYVI_ALLOW_COMMIT is set (by shell wrapper or explicit override)
[ -n "$TYVI_ALLOW_COMMIT" ] && exit 0

# Block
TYVI_ROOT="$(git rev-parse --show-toplevel)"
echo "Direct commit to tyvi project blocked." >&2
echo "" >&2
echo "To commit to the devspace project:" >&2
